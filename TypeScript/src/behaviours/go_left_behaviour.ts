...
/**
 * Walk left (-X) forever, one tile per tick. No pathfinding, no goal: a probe
 * for seeing what is out that way and how far the bot gets before something
 * stops it.
 */
export class GoLeftBehaviour implements IBehaviour {
  public readonly name = "go-left";
  ...
  public getNextAction(
    ...
    if (this.lastPosition && this.lastPosition.X === pos.X && this.lastPosition.Y === pos.Y) {
      this.blockedTicks++;
      console.log(`[${this.tag}] Blocked going left at ${pos.X},${pos.Y} for ${this.blockedTicks} tick(s).`);
    } else {
      if (this.blockedTicks > 0) {
        console.log(`[${this.tag}] Moving again, now at ${pos.X},${pos.Y}.`);
      }
      this.blockedTicks = 0;
      if (state.CurrentTick % GoLeftBehaviour.LOG_EVERY === 0) {
        console.log(`[${this.tag}] Going left, at ${pos.X},${pos.Y}.`);
      }
    }
